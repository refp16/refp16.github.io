# This rakefile is mostly from the Jekyll-Bootstrap project but I have adapted
...
# Usage: rake publish
# Credit: 
# https://github.com/gummesson/jekyll-rake-boilerplate/blob/master/Rakefile
# with minor changes.
# TODO: Rakefile should check if draft is opened and emit a warning.
desc "Move a post from _drafts to _posts"
task :publish do
  files = Dir["#{CONFIG['drafts']}/*.#{CONFIG['draft_ext']}"]
  files.each_with_index do |file, index|
    puts "#{index + 1}: #{file}".sub("#{CONFIG['drafts']}/", "")
  end
  print "> "
  number = $stdin.gets
  if number =~ /\D/
    filename = files[number.to_i - 1].sub("#{CONFIG['drafts']}/", "")
    date = (ENV['date'] ? Time.parse(ENV['date']) : Time.now).strftime('%Y-%m-%d')
    FileUtils.mv("#{CONFIG['drafts']}/#{filename}", "#{CONFIG['posts']}/#{date}-#{filename}")
    puts "#{filename} was moved to '#{CONFIG['posts']}'."
  else
...
# rake watch[number]
# rake watch["drafts"]
desc "Build, serve and watch the site (with post limit or drafts)"
task :watch, :option do |t, args|
  option = args[:option]
  puts "If build goes OK, check site at localhost:4000"
  if option.nil? or option.empty?
    ...
    if option == "drafts"
      ...
      system "bundle exec jekyll serve --watch --limit_posts #{option}"
    end
  end
end
